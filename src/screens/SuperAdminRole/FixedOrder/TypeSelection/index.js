...
const steps = ['جزییات سفارش', 'ثبت'];

export default function FixedOrderCreation({ title, image, type, teeth = [], color = '#ff0000', onSubmit }) {
  ...
  const onOpen = () => {
    if (teeth.length === 0) {
      return toast.warning('لطفا دندان های مورد نظر را انتخاب کنید');
    }
    return setOpen(true);
  };
    ...
    <>
      <Card variant="outlined" sx={{ width: '100%' }}>
        <CardActionArea onClick={onOpen} sx={{ height: '100%' }}>
          <CardMedia alt="Fixed" image={image} sx={{ pt: '60%', m: 1, borderRadius: 2 }} />
          <CardHeader
            sx={{ p: 1 }}
            title={title}
            subheader={
      ...
      <NewDialog open={open} onClose={onClose} maxWidth="xl">
        <NewDialogTitle
          title={<Typography variant="subtitle1">{steps[activeStep - 1]}</Typography>}
          onClose={onClose}
        />
        <NewDialogContent>
          <Box height={700}>
            <Steps step={activeStep} formData={formData} onChange={onChange} />
          </Box>
        </NewDialogContent>
        <NewDialogActions>
          <Stack width="100%">
...
const Steps = ({ step, formData, onChange }) => {
  switch (step) {
    case 1:
      return (
        <Stack rowGap={2} p={1}>
