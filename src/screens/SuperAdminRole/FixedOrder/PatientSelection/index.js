...
export default function MainModel({ patient, onChange }) {
  ...
  return (
    <Stack p={1} rowGap={1}>
      ...
      <Users patient={patient} data={result} loading={loading} onChange={onChange} />
      {!username && (
        <LoadingMore
          total={total}
...
function Users({ patient, data, loading, onChange }) {
    ...
    <Grid container>
      {data.map((item, i) => (
        <Grid key={i} item xs={12} sm={6} md={4} p={0.5}>
          <Model isSelected={patient?.id === item.id} data={item} onChange={onChange} />
        </Grid>
      ))}
    </Grid>
  );
...
function Model({ isSelected, data, onChange }) {
  ...
  return (
    ...
    >
      <CardActionArea onClick={() => onChange({ formData: { patient: data } })}>
        <CardHeader
          subheader={cellphone}
